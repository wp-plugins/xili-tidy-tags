...
class xili_tidy_tags_admin extends xili_tidy_tags {
	...
	function xili_add_pages() {
			...
			$this->thehook0 = add_object_page( sprintf(__('%s groups','xili_tidy_tags'), $this->tags_name), sprintf(__('Tidy %s','xili_tidy_tags'), $this->tags_name ) , '', 'xili-tidy-tags'.$pre, array(&$this,'top_tidy_menu_title'), plugins_url( 'images/xilitidy-logo-16.jpg', $this->file_file ) ); //1.5.5
	...
	function top_tidy_menu_title () { // again with wp3.0 instead '' call v1.3.4
		...
		<h4><a href="http://dev.xiligroup.com/xili-tidy-tags" title="Plugin page and docs" target="_blank" style="text-decoration:none" ><img style="vertical-align:middle" src="<?php echo plugins_url( 'images/xilitidy-logo-32.jpg', $this->file_file ) ; ?>" alt="xili-tidy-tags logo"/></a> - © <a href="http://dev.xiligroup.com" target="_blank" title="<?php _e('Author'); ?>" >xiligroup.com</a>™ - msc 2009-13 - v. <?php echo XILITIDYTAGS_VER; ?></h4>
		</div>
		<?php
 	}
	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xtt_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( __( 'Previous source file is splitted for admin.', 'xili_tidy_tags')). ',</p>';
...
// dummy params 3.5 ;
	...
	function xili_langs_import_terms () {
		...
		$theids = term_exists ( $this->xili_settings['tidylangsgroupname'], $this->tidy_taxonomy); // impossible to use get_term as formerly !!!
		if (!$theids) {
			$theids = wp_insert_term( $term, $this->tidy_taxonomy, $args); 
		...
		$listlanguages = get_terms(TAXONAME, array('hide_empty' => false )); 
		foreach ($listlanguages as $language) {
			$args = array( 'alias_of' => '', 'description' => $language->description, 'parent' => $theids['term_id'], 'slug' =>$language->slug.'-'.$this->xili_settings['tidylangsgroup']  ); // slug to be compatible with former release of WP
	...
	function xili_tags_row( $tagsnamelike='', $tagsnamesearch='' ) {
		...
		foreach ( $listtags as $tag ) {
			...
			echo '<tr><td>ERROR in TAXONOMY: '. $this->tidy_taxonomy .'</td></tr>';
		}
	}
