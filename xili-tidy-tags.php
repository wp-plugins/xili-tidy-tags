...
define('XILITIDYTAGS_VER','0.9.4'); /* used in admin UI */
...
class xili_tidy_tags {
	...
	function xili_tags_grouping ($post_ID) {
		if (!class_exists('xili_language')) return ; /* only used if present */
			...
			$listlanggroups = get_terms(TAXOTIDYTAGS, array('hide_empty' => false,'parent' => $langgroupid));
			if ($listlanggroups) {
				foreach ($listlanggroups as $curgroup) { 
					$langsgroup[] = $curgroup->term_id;
				}
				$langsgroup[] = $langgroupid; /* add group parent */
