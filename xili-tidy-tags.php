...
# 1.5.1 - 101127 - popup for groups in widget
...
define('XILITIDYTAGS_VER','1.5.1'); /* used in admin UI */
...
function xili_tidy_tag_cloud( $args = '' ) {
	...
	if (($tagsgroup == '' && $tagsallgroup == '' ) || !function_exists('get_terms_of_groups_new')) {
		...
		$tags = get_terms_of_groups_new ( $group_id, TAXOTIDYTAGS,'post_tag',array_merge( $r, array( 'orderby' => 'count', 'order' => 'DESC' )), false ); 
	}	
...
function get_terms_of_groups_new ( $group_ids, $taxonomy, $taxonomy_child, $order = '', $not = false ) {
		...
		//$not = true; fixe DISTINCT if 2 groups 1.5.1
		if ($not === false ) {
 ...
 * @since 1.3.3  
 * @updated 1.5.0 rewritten as extends - 1.5.1 popups
 */
...
class xili_tidy_tags_cloud_multiple_widgets extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( '' != $instance['thecondition'] && function_exists( $thecondition ) ) {
 		...
 		if ( $condition_ok ) {
			...
			if ('the_curlang' == $instance['tagsgroup']) {
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['thecondition'] = strip_tags(stripslashes($new_instance['thecondition'])); // 1.5.1
	...
	function form( $instance ) {
		...
		<fieldset style="margin:2px; padding:3px; border:1px solid #ccc;" >
			<label for="<?php echo $this->get_field_id('thecondition'); ?>"><?php _e('Condition','xili-language-widget'); ?></label>: <input class="widefat" id="<?php echo $this->get_field_id('thecondition'); ?>" name="<?php echo $this->get_field_name('thecondition'); ?>" type="text" value="<?php echo $thecondition; ?>" />
	( <input id="<?php echo $this->get_field_id('theparams'); ?>" name="<?php echo $this->get_field_name('theparams'); ?>" type="text" value="<?php echo $theparams; ?>" /> )
		...
		<p><small>© xili-tidy-tags v. <?php echo XILITIDYTAGS_VER; ?></small></p>
	<?php
	}
}
