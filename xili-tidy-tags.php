...
define('XILITIDYTAGS_VER','1.4.2'); /* used in admin UI */
...
class xili_tidy_tags {
	...
	function xili_tidy_tags_settings() { 
		...
		$themessages[1] = __('A new group was added.','xili-language');
		$themessages[2] = __('NO new group was added.','xili-language');
		$themessages[3] = __('A group was updated.','xili-language');
		$themessages[4] = __('A group was deleted.','xili-language');
		$themessages[5] = __('The languages groups was added.','xili-language');
		$themessages[6] = $optionmessage ;
		
		/* form datas in array for do_meta_boxes() */
		$data = array('xili_tidy_tags_page' => $xili_tidy_tags_page,'message' => $message, 'action'=>$action, 'formtitle'=>$formtitle, 'tagsgroup'=>$tagsgroup, 'submit_text'=>$submit_text,'cancel_text'=>$cancel_text, 'term_id'=>$term_id);
		?>
	...
	function xili_langs_import_terms () {
		...
		if (is_wp_error($resgroup)) {$theids = get_term($term, TAXONAME);} else { $theids = $resgroup;}
		$listlanguages = get_terms(TAXONAME, array('hide_empty' => false)); 
		foreach ($listlanguages as $language) {
			$args = array( 'alias_of' => '', 'description' => $language->description, 'parent' => $theids['term_id']);
	...
	function xili_tags_group_row() {
		...
		if ( $listtagsgroups == array() ) {
			...
			if (defined('TAXONAME')) { /* xili-language is present */
				...
				$resgroup = wp_insert_term( $term, TAXOTIDYTAGS, $args);
				// @since 1.4.2 - no warning if no groups at init
				/* to reset cached from empty first query  as lines #869 taxonomy.php (#663 wp2.9.1 */
				$heredefaults = array('orderby' => 'name', 'order' => 'ASC',
					'hide_empty' => 0, 'exclude' => array(), 'exclude_tree' => array(), 'include' => array(),
					'number' => 0, 'fields' => 'all', 'slug' => '', 'parent' => '',
					'hierarchical' => false, 'child_of' => 0, 'get' => 'all', 'name__like' => '',
					'pad_counts' => false, 'offset' => 0, 'search' => ''
				); 
				// choice of false or 0 is very important for serialize !!!
				extract($heredefaults, EXTR_SKIP); 
				
				$filter_key = ( has_filter('list_terms_exclusions') ) ? serialize($GLOBALS['wp_filter']['list_terms_exclusions']) : ''; 
				$key = md5( serialize( compact(array_keys($heredefaults)) ) . serialize( array(TAXOTIDYTAGS) ) . $filter_key );
				$last_changed = wp_cache_get('last_changed', 'terms');
	
				//echo 'xili-ck='.serialize(compact(array_keys($heredefaults)));
	
				$cache_key = "get_terms:$key:$last_changed";  //echo 'xili-t='. $cache_key ;
				wp_cache_delete( $cache_key, 'terms' );
				
				// now the cache is empty and next query will done and give new group tags
			}	
 ...
 * @since 0.9.2  
 * @updated 1.3.3 - displayable as list
 */
class xili_tidy_tags_cloud_multiple_widgets {
	...
	function widget($args, $widget_args = 1) {
		...
		if ( !isset($options[$number]) )
			return;
		$domain = (function_exists('the_theme_domain')) ? the_theme_domain(): THEME_TEXTDOMAIN; /* wpmu and wp2.9.x*/
		$cloudsargs = array();
		echo $before_widget.$before_title.__($options[$number]['title'],$domain).$after_title;
		...
		$cloudsargs[] = 'format='.$options[$number]['displayas']; /* since 1.3.3 */
	...
	function widget_options($widget_args) {
		...
		if ( !$updated && !empty($_POST['sidebar']) ) {
			...
			foreach ( (array) $_POST['xili_tdtc_widget'] as $widget_number => $widget_text ) {
				if (isset($widget_text['submit'])) {
					...
					$options[$widget_number]['displayas'] = strip_tags(stripslashes($widget_text['displayas']));
				}
			}
		...
		if ( -1 == $number ) {
			...
			$displayas = $options[$number]['displayas'];
		}
		...
		echo '</fieldset>';
		echo '<fieldset style="margin:2px; padding:3px; border:1px solid #ccc;"><legend>'.__('Display as','xili_tidy_tags').'</legend>';
		echo '<select name="xili_tdtc_widget['.$number.'][displayas]" id="xili_tdtc_widget_order-'.$number.'" style="width:100%;">';
