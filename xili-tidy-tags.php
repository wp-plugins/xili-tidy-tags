...
# 0.9.5 - 090527 - options to manage capabilities to group tags (enable for editors) - =& instantiation
...
define('XILITIDYTAGS_VER','0.9.5'); /* used in admin UI */
...
class xili_tidy_tags {
	 ...
	 *
	 * @since 0.8.0
	 * @updated 0.9.5 - menu without repeat of main title, levels with new caps set by plugin
	 *
	 */
	function xili_add_pages() {
		 
		  	$this->thehook0 = add_object_page(__('Tags groups','xili_tidy_tags'), __('Tidy Tags','xili_tidy_tags'), -1, __FILE__, array(&$this,'top_tidy_menu_title'));		  	
	...
	function  on_sub_sidebox_3_content($data=array()) { 
		...
		<fieldset style="margin:2px; padding:3px; border:1px solid #ccc;"><legend><?php _e('Groups selection','xili_tidy_tags'); ?></legend>
			...
			$listterms = get_terms(TAXOTIDYTAGS, array('hide_empty' => false));
				...
				<br /> <p class="submit"><input type="submit" id="subselection" name="subselection" value="<?php _e('Sub select…','xili_tidy_tags'); ?>" /></p></fieldset><?
	}
	/**
