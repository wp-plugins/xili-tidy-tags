...
define('XILITIDYTAGS_VER','1.6.0'); /* used in admin UI */
...
class xili_tidy_tags {
	...
	function init_plugin() { 
		...
		$res = is_term ( LANGSTAGSGROUPNAME, $this->tidy_taxonomy );
	...
	function xili_add_tag_form() { 
		...
		$checkline .='<br /><br /><small>'.__('© by xili-tidy-tags.','xili_tidy_tags').'</small>';
			echo '<div style="margin:2px; padding:3px; border:1px solid #ccc;"><label>'.sprintf( __('%s groups','xili_tidy_tags'), $this->tags_name ) . ':</label><br />'.$checkline.'</div>';	
	}
	...
	function xili_edit_tag_form( $tag ) { 
		...
		$checkline .='<br /><br /><small>'.__('© by xili-tidy-tags.','xili_tidy_tags').'</small>';
			echo '<div style="margin:2px; padding:3px; border:1px solid #ccc;"><label>'.sprintf( __('%s groups','xili_tidy_tags'), $this->tags_name ) .':<br /></label><br />'.$checkline.'</div>';	
	}
				...
				foreach ( $list_tags as $tag ) { /* test if the tag is owned by a group */ 
					...
					if ($nbchecked == false) { 
						if ($post_curlang == false) { /* add to group parent */
							...
							$res = is_term ($post_curlang,$this->tidy_taxonomy);
	...
	function xili_add_pages() {
			...
			$this->thehook0 = add_object_page( sprintf(__('%s groups','xili_tidy_tags'), $this->tags_name), sprintf(__('Tidy %s','xili_tidy_tags'), $this->tags_name ), -1, 'xili-tidy-tags'.$pre, array(&$this,'top_tidy_menu_title'), plugins_url( 'images/xili-favicon.ico', __FILE__ ) ); //1.5.5
				 	
		  	$this->thehooka = add_submenu_page('xili-tidy-tags'.$pre, sprintf( __('%s groups','xili_tidy_tags'), $this->tags_name),__('Info for SuperAdmin','xili_tidy_tags'),-1, 'xili-tidy-tags'.$pre, array(&$this,'top_tidy_menu_title'));
	...
	function on_load_page() {
			...
			add_meta_box($pre.'xili_tidy_tags-sidebox-3', __('Info','xili_tidy_tags'), array(&$this,'on_sidebox_2_content'), $this->thehook , 'side', 'core');
			...
			if (current_user_can( 'administrator'))
				add_meta_box($pre.'xili_tidy_tags-sidebox-4', __('Capabilities','xili_tidy_tags'), array(&$this,'on_sidebox_4_content'), $this->thehook , 'side', 'core');
	...
	function on_load_page2() {
			...
			$pre = ( $this->post_tag == 'post_tag') ? ''  : $this->post_tag.'_' ;
			add_meta_box($pre.'xili_tidy_tags-sidebox-1', __('Message','xili_tidy_tags'), array(&$this,'on_sidebox_1_content'), $this->thehook2 , 'side', 'core');
			add_meta_box($pre.'xili_tidy_tags-sidebox-3', __('Info','xili_tidy_tags'), array(&$this,'on_sidebox_2_content'), $this->thehook2 , 'side', 'core');
	}
...
//echo $query ;
	...
	function  on_sidebox_3_content($data=array()) { 
	 	...
	 	if ( $this->post_tag == 'post_tag' ) { // 1.5.5
		 	...
		 	foreach ($categories as $cat) {
		 		$catinside = is_term ($cat->slug,$this->tidy_taxonomy);
	 	...
	 	if ( !class_exists('xili_language') ) { ?>
	 		...
	 		$res = is_term (LANGSTAGSGROUPNAME,$this->tidy_taxonomy); //print_r($res); echo (is_array($res)) ? "arr" : '?' ; 
	...
	function xili_tidy_tags_settings() { 
		...
		add_meta_box('xili_tidy_tags-normal-1', sprintf( __('Groups of %s','xili_tidy_tags'), $this->tags_name), array(&$this,'on_normal_1_content'), $this->thehook , 'normal', 'core'); /* list of groups*/
	...
	function xili_langs_import_terms () {
		...
		$theids = is_term (LANGSTAGSGROUPNAME,$this->tidy_taxonomy); // impossible to use get_term as formerly !!!
		if (!$theids) {
			$theids = wp_insert_term( $term, $this->tidy_taxonomy, $args);  //print_r($theids);
		...
		foreach ($listlanguages as $language) {
			...
			$ifhere = is_term ( $language->name, $this->tidy_taxonomy );
			if (!$ifhere) {
	...
	function xili_tags_row($tagsnamelike='',$tagsnamesearch='') {
		...
		foreach ( $listtags as $tag ) {
			...
			foreach ($listgroupids as $groupid) {
				if (is_object_in_term($tag->term_id,$this->tidy_taxonomy,$groupid)) {
	...
	function xili_tidy_tags_assign () { 
		...
		add_meta_box( 'xili_tidy_tags-normal-1', sprintf(__('Tidy %s','xili_tidy_tags'), $this->tags_name ), array( &$this,'on_sub_normal_1_content' ), $this->thehook2 , 'normal', 'core'); /* list of tags*/
...
function xili_tidy_tag_cloud( $args = '' ) {
	...
	$defaults = array(
		...
		'tidy_taxonomy' => 'xili_tidy_tags', 'tidy_post_tag' => 'post_tag'
	);
	...
	if (($tagsgroup == '' && $tagsallgroup == '' ) || !function_exists('get_terms_of_groups_new')) {
		...
		if ($tagsgroup !='') {
			$groupterm = is_term($tagsgroup, $tidy_taxonomy);
			$group_id[] = $groupterm['term_id'];
		}
		if ($tagsallgroup !='') {
			$groupterm = is_term($tagsallgroup,$tidy_taxonomy);
			$group_id[] = $groupterm['term_id'];
		}
...
function xili_get_object_terms($object_ids, $taxonomies, $args = array()) {
	...
	foreach ($sub_groups as $tagsgroup) {
		if ($tagsgroup !='') {
			$groupterm = is_term($tagsgroup, $tidy_tags_taxo); //echo '----'.$tagsgroup;
	...
	extract($args, EXTR_SKIP); //print_r ($args);
...
function get_terms_of_groups_new ( $group_ids, $taxonomy, $taxonomy_child, $order = '', $not = false, $uncheckedtags = false ) {
		...
		//echo $query;
		$listterms = $wpdb->get_results( $wpdb->prepare( $query ) );
		if ( ! $listterms )
			return array();
...
class xili_tidy_tags_cloud_multiple_widgets extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $condition_ok ) {
			...
			$cloudsargs[] = 'tidy_taxonomy='.$instance['tidy_taxonomy'];
